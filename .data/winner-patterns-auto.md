...
Mientras tanto, se muestran los patrones de scripts grabados.
...
| vulnerabilidad_conexion | 1 | 0 | 0% |
| Validación (tu experiencia vale) → Oportunidad (hay demanda real) → Deseo (podés hacerlo en ratos libres) → Acción (registrate) | 1 | 0 | 0% |
| Identificación (55 años, sin tech) → Frustración compartida (métodos que no sirven) → Esperanza (la IA hizo lo técnico) → Acción (si ella pudo, yo puedo) | 1 | 0 | 0% |
| Escepticismo (¿vendés durmiendo?) → Prueba visual (las notificaciones) → Comprensión (el mecanismo) → Deseo (yo puedo hacer eso) | 1 | 0 | 0% |
| Sorpresa (40K búsquedas) → Oportunidad (nadie lo hace bien) → Deseo (la IA me lo arma) → Acción (quiero aprender) | 1 | 0 | 0% |
| Desconfianza (otro taller más) → Curiosidad (¿qué pasa adentro?) → Confianza (es real) → Decisión (por $5 lo pruebo) | 1 | 0 | 0% |
| Identificación (eso me pasa) → Revelación (mi conocimiento vale) → Posibilidad (la IA me ayuda) → Decisión (quiero hacerlo) | 1 | 0 | 0% |
| Reconocimiento (yo sé de autos) → Sorpresa (vale plata online) → Deseo (la IA me lo arma) → Urgencia (quiero aprender) | 1 | 0 | 0% |
| Reconexión (ya me viste, seguí escuchando) → idea concreta (manualidades como producto digital) → proceso simple (IA + Hotmart) → cambio de creencia (no necesitás ferias, necesitás una guía) | 1 | 0 | 0% |
| Identificación (yo también uso IA para boludeces) → frustración suave (estás desperdiciando la herramienta) → curiosidad (qué pasa si la uso diferente) → esperanza concreta (ejemplo fitness) → cambio de creencia (saber pedir = saber ganar) | 1 | 0 | 0% |
